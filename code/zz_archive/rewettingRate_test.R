...
metrics_fun <- function(n){
  ...
  for (i in p_list){
    ...
    output = output %>% add_row(event,gage,cluster,wet_date,wetLengthDate,wetLength,wetLengthQ,wetPeak,wetPeakLength,wetPeakQ,Q_threshold,rewetting_rate,p_value)
  }
  ## Call output
  output
}
